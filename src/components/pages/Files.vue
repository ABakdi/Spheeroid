         ...
         let connected = ref([
             {
                 id:'123-456-789-abcd',
                 name: 'Xoid',
                 progress:'25%(25MB/100MB)',
                 direction: 'incoming'
             },
             {
                 id:'123-9865-878-c98',
                 name: 'alphoid',
                 progress:'50%(1.1GB/2.2GB)',
                 direction: 'outgoing'
             },
             {
                 id:'123-456-abcd-fd88',
                 name: 'astroid',
                 progress:'10%(1MB/100MB)',
                 direction: 'outgoing'
             },
         ])
     ...
     methods: {
         Filter: function(e){
             ...
             let filtered = this.connected.filter((p)=>{
                 return !(p.direction == 'incoming' && !this.remote_requests || p.direction == 'outgoing' && !this.my_requests)
             })
    ...
    <div class="search-bar">
        ...
        <div class="check">
            ...
            <div class="check-el rem-req">
                ...
                <label for="remReq">receiving</label>
            </div>
        </div>
    </div>
    <div class="resaults">
        <panel class="search-resault" v-for="peer in filtered" :color="peer.direction == 'incoming'? 'prime':'sec'">
            <template #main>
                <div class="feature">
                ...
                <div class="feature">
                    ...
                    <span class="val">{{peer.progress}}</span>
                </div>
            </template>
            <template #side>
                <x-button class="action" color="prime">details</x-button>
            </template>
            <template #bottom>
                <div class="progress-bar">
                    <div class="progress"></div>
                </div>
                <x-button class="control"><i class="fa-solid fa-stop"></i></x-button>
                <x-button class="control"><i class="fa-solid fa-times"></i></x-button>
            </template>
        </panel>
    </div>
  </div>
...
<style scoped>
 .progress-bar{
     width: 90%;
     border: 1px solid rgba(95, 133, 219, 1);
 }
 .control{
     width: 3%;
 }
 .progress{
     width: 20%;
     height: 100%;
     background-color: rgba(76, 93, 133, 1);
 }
  .container {
      width: 97%;
      min-height: 95%;
